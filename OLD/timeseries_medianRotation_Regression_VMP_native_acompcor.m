...
plotDir = dataDir;
plotFilename = strcat(plotDir,'S_',sprintf('%.4d',subj_idx),'_',cfgMain.task,'_chancePLVRotationhistogram_NativeAcompcor');

filename_brain_mask = ['/mnt/fast_scratch/StomachBrain/data/allpreprocRest/sub-',sprintf('%.4d',subj_idx),'_ses-session1_task-loc_run-001_space-MNI152NLin2009cAsym_desc-brain_mask.nii.gz'];
indBrain= logical(niftiread(filename_brain_mask));
outsideBrain = ~indBrain;

%% Load data and outputfilename
BOLDPhasesTimeseriesFilename = [dataDir,'sub-',sprintf('%.4d',subj_idx),'_BOLD_Native_filtered_acompcor_phases']; 
medianRotationFilename = [dataDir,'sub-',sprintf('%.4d',subj_idx),'_medianRotationNativeAcompCor']; 
...
RPLV = zeros(length(indexRotations),109*129*108); % R from rotated
%iterate through all rotation and calculate PLV

for iRotation = 1 : length(indexRotations)
    

PLV = zeros (109,129,108);
PLV = PLV(:);
...
medianPLV = reshape (medianPLV,109,129,108);
